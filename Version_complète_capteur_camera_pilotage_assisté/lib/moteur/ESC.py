...
class ESC(Moteur):
    ...
    def __init__(self, pwm, min_v=1000, max_v=2000, centre=1500, freq=50, arm_sequence=False):
        # Initialisation de la classe parent (PWM + limites)
        super().__init__(pwm, min_v, max_v)
        
        # Valeur de position centrale de l'hélice 
        self.centre = centre
    ...
    def __pulse_to_duty(self, pulse):
    ...
    def arm(self):
        ...
        self.__set_us(self.min)
        time.sleep(1.0)
        # Signal maximum (calibration)
        self.__set_us(self.max)
        time.sleep(1.0)
